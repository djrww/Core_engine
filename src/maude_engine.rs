...
impl MaudeModule {
    ...
    pub fn export_maude(&self, pool: &DagPool) -> String {
        ...
        writeln!(out, "--- ==========================================").unwrap();
        writeln!(out, "--- Maude Formal System Specification: {}", self.name).unwrap();
        writeln!(out, "--- Generated by Core_engine cl0r0 Rewriting Logic").unwrap();
        writeln!(out, "--- ==========================================").unwrap();
        writeln!(out, "mod {} is", self.name).unwrap();
        writeln!(out, "  sorts {} .", self.sorts.join(" ")).unwrap();
        ...
        for eq in &self.equations {
            ...
            if let Some(ref lbl) = eq.label {
                writeln!(out, "  eq [{}] : {} = {} .", lbl, lhs_str, rhs_str).unwrap();
            } else {
                writeln!(out, "  eq {} = {} .", lhs_str, rhs_str).unwrap();
            }
        }
        ...
        for rl in &self.rules {
            ...
            writeln!(out, "  rl [{}] : {} => {} .", rl.label, lhs_str, rhs_str).unwrap();
        }
