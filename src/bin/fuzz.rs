...
//!
//! 審計 F-08 修復:迭代規模讀取環境變量(硬編碼值僅為預設):
//!   `FUZZ_ITERATIONS`(主輪,預設 4000,CI 設 100000)
//!   `FUZZ_LEGAL_ITERATIONS`(預設 800)/ `FUZZ_HALF_ITERATIONS`(預設 1500)
//!   `FUZZ_EDIT_ITERATIONS`(預設 500)/ `FUZZ_REPARSE_ITERATIONS`(預設 200)
//!   `FUZZ_L8_ITERATIONS`(預設 300)
//! scheduled_fuzz.yml 注入的 FUZZ_ITERATIONS=100000 自此真正生效,
//! CI 與代碼回到單一真相源。
...
fn main() {
    println!(
        ...
        iters("FUZZ_ITERATIONS", 4000),
        iters("FUZZ_LEGAL_ITERATIONS", 800),
        iters("FUZZ_HALF_ITERATIONS", 1500),
        iters("FUZZ_EDIT_ITERATIONS", 500),
        iters("FUZZ_REPARSE_ITERATIONS", 200),
        iters("FUZZ_L8_ITERATIONS", 300),
    );
    ...
    let n1 = iters("FUZZ_ITERATIONS", 4000);
    for i in 0..n1 {
        let src = if i % 3 == 0 {
            gen_legal(&mut rng)
        } else {
    ...
    // 合法程式的密集樣本
    let n_legal = iters("FUZZ_LEGAL_ITERATIONS", 800);
    for _ in 0..n_legal {
    ...
    // ---------- L7b:寫一半的檔案(注入式) ----------
    let n_half = iters("FUZZ_HALF_ITERATIONS", 1500);
    for _ in 0..n_half {
    ...
    // ---------- 編輯單體 ----------
    let n_edits = iters("FUZZ_EDIT_ITERATIONS", 500);
    for _ in 0..n_edits {
        let src = gen_legal(&mut rng);
    ...
    let n_rep = iters("FUZZ_REPARSE_ITERATIONS", 200);
    ...
    let n_l8 = iters("FUZZ_L8_ITERATIONS", 300);
    for _ in 0..n_l8 {
