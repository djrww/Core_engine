...
fn main() {
    ...
    // 生成 Knuth-Bendix 關鍵對短證(F-03:由機械核驗實錄的臨界對會合見證構造)
    let kb_states = vec![cl0r0::testkit::fixtures::overlapping_pair()];
    let kb_sn_witness = SNWitness::LivenessScopeBounded {
        max_span_len: 4,
        storages: 1,
    };
    let kb_report = check_confluence_with_mode(
        &kb_states,
        CheckerMode::Newman {
            sn_witness: kb_sn_witness.clone(),
        },
        6,
    );
    let kb_cert = CPFCertificate::new_knuth_bendix(
        "CL0-KB-Core",
        &kb_sn_witness.description(),
    ...
    if dd_valid && kb_valid {
        ...
        println!(
            "  [✓] CPF-KB 證書出具成功 -> proofs/cl0_kb_confluence.cpf ({} 對臨界對見證全數閉合)",
            kb_cert.critical_pairs_count()
        );
        total_witnesses += 2;
    } else {
        eprintln!("  [✗] CPF 證書驗證失敗！");
    ...
    // D-01:引用 testkit 夾具(AState::new 自動補 id,語義等價)
    let astate = cl0r0::testkit::fixtures::two_event_state(0, 4, 2, 6);
