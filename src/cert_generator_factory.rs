...
impl CertGeneratorFactory {
    ...
    pub fn run_factory_production(batch_size: usize, seed: u64) -> GeneratedArtifacts {
        ...
        for i in 0..batch_size {
            ...
            let astate = crate::testkit::fixtures::two_event_state(s1, e1, s2, e2);
            ...
            if report.certified {
                ...
                // F-03:有峰值 ⇒ KB 短證攜帶機械實錄的會合見證;
                // 零峰值 ⇒ 如實以 Rosen 正交性申報,而非空殼 KB 計數。
                let cert_kb = if report.kb_critical_pair_witnesses.is_empty() {
                    CPFCertificate::new_orthogonal(&mod_name)
                } else {
