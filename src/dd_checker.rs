...
/// 將狀態編成可讀的會合見證字串(基於規範化五元組鍵)
fn state_witness_string(s: &AState) -> String {
    format!("{:?}", s.canon_key())
}
...
pub struct DDReport {
    ...
    /// F-03:Newman 通道機械實錄的臨界對會合見證(峰值兩側 + 會合點)。
    /// KB 短證從此攜帶見證實體,而非僅有可捏造的計數。
    pub kb_critical_pair_witnesses: Vec<CriticalPairWitness>,
}
...
pub fn check_confluence_with_mode(
    ...
    let mut kb_critical_pair_witnesses = Vec::new();
                ...
                if let (Some(s_left), Some(s_right)) = (apply_rule(s, r1), apply_rule(s, r2)) {
                    let joined = match &mode {
                        CheckerMode::Newman { .. } => {
                            ...
                            // F-03:機械實錄會合見證(兩側峰值 + 共同歸約)。
                            match find_unconstrained_valley(&s_left, &s_right, depth_limit) {
                                Some(joined_state) => {
                                    kb_critical_pair_witnesses.push(CriticalPairWitness::new(
                                        &state_witness_string(&s_left),
                                        &state_witness_string(&s_right),
                                        &state_witness_string(&joined_state),
                                    ));
                                    true
                                }
                                None => false,
