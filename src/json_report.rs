...
use crate::cpf_cert::{CPFCertificate, CriticalPairWitness};
use crate::edit::Edit;
...
impl JsonDiagnosticPipeline {
    ...
    pub fn analyze_and_generate_report(
        ...
        // F-03/F-04:錯誤報告的隨附證書不應偽稱 KB 合流證明。此處輸出的
        // 是零約束的內部 DD 證書(空偏序 = 不作任何良基性宣稱),僅作歸檔。
        let cert = CPFCertificate::new_decreasing_diagrams("CL0-JSON-Report", vec![], vec![]);
    ...
    pub fn execute_json_repair_to_fixpoint(
        ...
        let final_report = FullJsonErrorReport {
            ...
            cpf_certificate_xml: Some(
                // F-03:修復閉環證書攜帶真實會合見證
                // (初始 E0502 錯誤,自動修復補丁,重析驗證後的 0 錯誤態)。
                CPFCertificate::new_knuth_bendix(
                    "CL0-Repair-Fixpoint",
                    "LivenessBounded",
                    vec![CriticalPairWitness::new(
                        "E0502@10-20",
                        "shorten-repair-patch",
                        "0-errors-after-reparse",
                    )],
                )
                .to_cpf_xml(),
            ),
